...
const Actions = {
  fetchedDevices: createAction('FETCHED_DEVICES', resolve => (p: Devices) => resolve(p)),
  fetchedDevicesError: createAction('FETCHED_DEVICES_ERROR', resolve => (p: Error) => resolve(p)),
  toggledDevice: createAction('TOGGLED_DEVICE', resolve => (p: Lamp) => resolve(p)),
  setLayer: createAction('SET_LAYER', resolve => (p: string) => resolve(p)),
  fetchedPresets: createAction('FETCHED_PRESETS', resolve => (p: {
    deviceName: string,
    presets: Presets,
  }) => resolve(p)),
  setActivePreset: createAction('SET_ACTIVE_PRESET', resolve => (p: { deviceName: string, value: string }) => resolve(p)),
};
...
export const executePreset = (preset: string): ThunkResult => async (dispatch) => {
...
export const executeShortcut = (shortcut: string): ThunkResult => async (dispatch) => {
  await axios.post('/action', {
    action: 'shortcut',
    shortcut,
  });
...
export const updateDevice = (deviceName: string): ThunkResult => async dispatch => {
...
export const savePreset = (device: string, raw_string: string): ThunkResult => () =>
